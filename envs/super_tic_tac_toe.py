...
class SuperTicTacToeEnv(object):
    ...
    def reset(self):
        self.winner = -1
        self.player_id = random.randint(0, 1)
        self.board = np.zeros((6, 6, 2))
        self.is_game_over = False
    ...
    def render(self, board=None, mark: int = 0):
        if board is None:
            board = self.board
...
def evaluate_tic_tac_toe(agents, env: SuperTicTacToeEnv, num_episodes: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    assert len(agents) == 2
    returns, lengths, ties, info = [], [], [], {}
    for _ in range(num_episodes):
        ...
        while not done:
            observations = board[:, :, [player_id, 1 - player_id]][np.newaxis, :]
            legal_actions = la[np.newaxis, :]
            action = agents[player_id].sample_actions(observations, legal_actions)
